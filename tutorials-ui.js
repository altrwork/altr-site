(() => {
  ...
  const searchInput = document.querySelector('.tutorials-search');
  const checkboxes = Array.from(document.querySelectorAll('.tutorials-filter-check'));
  const clearBtn = document.getElementById('tutorials-clear');
  const viewBtns = document.querySelectorAll('.tutorials-view-btn');
  ...
  function applyFilters() {
    ...
    cards.forEach(card => {
      ...
      const desc = card.querySelector('.tutorial-card-desc')?.textContent.toLowerCase() || '';
